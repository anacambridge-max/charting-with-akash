...
// Vercel Hobby + Fluid Compute supports a maximum function duration of 300s.
// The browser reconnects automatically when the socket reaches the platform limit.
...
export async function GET() {
  ...
  return experimental_upgradeWebSocket((ws) => {
    ...
    const startStreamer = (symbol: string) => {
      ...
      streamer.on("error", () =>
        ws.send(JSON.stringify({ type: "error", message: "Upstox market feed error" })),
      );
    ...
    ws.on("message", (data: WebSocketData) => {
      ...
      if (message?.type === "subscribe") {
        startStreamer(String(message.symbol || "RELIANCE").toUpperCase());
      }
    });
