...
export async function GET(request: NextRequest) {
  ...
  if (!token) return NextResponse.json({ error: "UPSTOX_ACCESS_TOKEN is not configured" }, { status: 500 });
  ...
  const allowed = ["1", "3", "5", "15"];
  if (!allowed.includes(interval)) return NextResponse.json({ error: "Supported intervals: 1, 3, 5, 15" }, { status: 400 });
